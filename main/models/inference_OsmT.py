...
from utils import read_overpass_split, OverpassDataset, read_kv_split
...
parser.add_argument('--model_name', default='codet5-base', type=str)
...
use_prefix = False
use_primary_kv = False
if 'prefix' in exp_name:
    use_prefix = True
if 'primary_kv' in exp_name:
    use_primary_kv = True
...
def run_inference(model_dir, model, tokenizer, split, beams):
    ...
    if use_primary_kv:  
        if use_prefix:
            ...
            prefix_tag = ''
        kv_texts = read_kv_split(opts.data_dir + f"/dataset.{split}", primary_kv=use_primary_kv, prefix=prefix_tag)
        # add key value to the last token of the input
        texts = [f"{t} {kv}" for t, kv in zip(texts, kv_texts)]
    ...
    if opts.compute_oqo:
        ...
        print('kv_overlap', metrics['kv_overlap'])
        print('xml_overlap', metrics['xml_overlap'])
