...
import random
@dataclass
class DataCollatorForT5MLM_bidir:
    """
...
def tokenize_function_bidir(examples, tokenizer, max_length):
...
def process_dataset_bidir(dataset_splits, args, tokenizer):
    final_datasets = {}

    for split, dataset_split in dataset_splits.items():

        dataset_split = dataset_split.map(
            tokenize_function_bidir,
            batched=True,
            fn_kwargs={
                'tokenizer': tokenizer,
